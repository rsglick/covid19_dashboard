    ...
    df = df.astype(
        {
            ...
            "new_cases": int,
            "new_deaths": int,
            "new_cases_rolling_mean": int,
            "new_deaths_rolling_mean": int,
        }
    )
    return df
...
def load_data() -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
    ...
    df_us_counties_data.loc[:, "date"] = pd.to_datetime(
        df_us_counties_data.loc[:, "date"]
    )
...
county_choice = st.selectbox(
    "Select", sorted(us_counties_data[county_state_conditon]["county"].unique())
)

condition = (county_state_conditon) & (
    us_counties_data[county_state_conditon]["county"] == county_choice
)
